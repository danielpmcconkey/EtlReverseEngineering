...
**All product artifacts live in EtlReverseEngineering, NOT MockEtlFrameworkPython.**
The ETL framework accesses generated code via tokenized paths in `control.jobs`,
e.g. `{token}/EtlReverseEngineering/jobs/42/artifacts/code/jobconf.json`.
No cross-repo writes, no commits mid-flight.
...
| `{OG_CURATED}` | OG curated output (read-only) | `/workspace/og-curated` |
